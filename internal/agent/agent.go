// Package agent is the core orchestration layer for Ultra AI agents.
//
...
var userAgent = fmt.Sprintf("Ultra/%s (https://github.com/asx8678/ultra)", version.Version)
...
type SessionAgentCall struct {
	...
	// event, so non-interactive clients (e.g. `ultra run`) don't
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil && a.canceledBySeq(call.SessionID, call.Accepted.seq) {
		...
		// `ultra run`, which ignores message events and blocks on
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "ultra auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("ULTRA_DISABLE_ANTHROPIC_CACHE")); t {
