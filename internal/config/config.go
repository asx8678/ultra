...
type ProviderConfig struct {
	...
	// When Models is empty and this is nil or true, Ultra auto-discovers
...
type MCPConfig struct {
	...
	// never issues a Mcp-Session-Id). When true, Ultra omits the
	...
	// server.
	//
	// When nil, Ultra auto-detects a set of known sessionless servers (see
	// IsSessionless); set it explicitly to override that detection.
	Sessionless *bool `json:"sessionless,omitempty" jsonschema:"description=Mark a sessionless MCP server (no Mcp-Session-Id) so Ultra skips the subscriptions/listen stream it would otherwise reject. Leave unset to auto-detect known sessionless servers (e.g. GitHub MCP),default=false"`
	...
	// Ultra picks the first free port from its default range.
...
type TUIOptions struct {
	...
	ExitBanner  ExitBanner  `json:"exit_banner,omitempty" jsonschema:"description=Exit banner style after quitting Ultra,enum=default,enum=compact,enum=none,default=default"`
}
...
// ExitBanner selects what Ultra prints after the TUI exits.
...
type Attribution struct {
	...
	GeneratedWith bool         `json:"generated_with,omitempty" jsonschema:"description=Add Generated with Ultra line to commit messages and issues and PRs,default=true"`
}
...
type Options struct {
	...
	SkillsPaths          []string    `json:"skills_paths,omitempty" jsonschema:"description=Paths to directories containing Agent Skills (folders with SKILL.md files),example=~/.config/ultra/skills,example=./skills"`
	TUI                  *TUIOptions `json:"tui,omitempty" jsonschema:"description=Terminal user interface options"`
	Debug                bool        `json:"debug,omitempty" jsonschema:"description=Enable debug logging,default=false"`
	DebugLSP             bool        `json:"debug_lsp,omitempty" jsonschema:"description=Enable debug logging for LSP servers,default=false"`
	DisableAutoSummarize bool        `json:"disable_auto_summarize,omitempty" jsonschema:"description=Disable automatic conversation summarization,default=false"`
	// DataDirectory is where Ultra keeps per-project state such as
...
// Config holds the configuration for ultra.
type Config struct {
