...
var descriptors = []Descriptor{
	// fabric_exec's outer envelope has no direct host effect, so it remains
	// available in read-only sessions. EffectRead grants no nested authority:
	// every nested action is classified and authorized independently by Fabric.
	{Name: "fabric_exec", Group: "fabric", Effects: EffectRead, DefaultEnabled: true},
