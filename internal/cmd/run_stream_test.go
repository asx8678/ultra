...
// with reason=tool_use and used to terminate `ultra run` early via
...
// subscribers (e.g. a TUI watching workspace events while `ultra
...
// non-nil error from `ultra run` so shells and CI catch it via
// exit status.
...
// queue scenario: `ultra run --continue` attaches to a session
